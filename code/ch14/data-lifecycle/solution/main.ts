// Solution for 14.7 課題14.7: 個人データの削除・保持・エクスポート・同意の抜けを再現して塞ぐ (★★★)
...
export function fixedErase(world: World, subjectId: string): DeletionRequest {
  const missing = INVENTORY.filter((entry) => !world.stores.has(entry.location));
  if (missing.length > 0) {
    throw new Error(`erase not implemented for: ${missing.map((entry) => entry.location).join(', ')}`);
  }
  const request = newDeletionRequest(subjectId, NOW);
  for (const entry of INVENTORY) {
    eraseAt(world, entry.location, subjectId, entry.erasure);
    request.targets[entry.location] = 'done';
  }
  request.state = 'done';
  return request;
}
...
export function fixedPurge(world: World, at: number, limit = 1_000): number {
  let purged = 0;
  for (const target of expired(world, at)) {
    if (purged >= limit) break;
    const item = rowsOf(world, target.location).find((candidate) => candidate.id === target.id);
    if (!item) continue;
    item.identifiers = {};
    purged += 1;
  }
...
export function fixedExport(world: World, subjectId: string): ExportBundle {
  const missing = EXPORTABLE_LOCATIONS.filter((location) => !world.stores.has(location));
  if (missing.length > 0) throw new Error(`export not implemented for: ${missing.join(', ')}`);
  const rows: ExportBundle['rows'] = [];
  for (const location of EXPORTABLE_LOCATIONS) {
    for (const item of rowsOf(world, location)) {
      if (item.subjectId !== subjectId) continue;
      rows.push({ location, id: item.id, ...(item.body === undefined ? {} : { body: item.body }) });
    }
  }
  return { subjectId, rows };
}
...
export function fixedDispatch(world: World, consents: ConsentStore, at: number): string[] {
  ...
  for (const item of rowsOf(world, 'db.users')) {
    ...
    if (!consentActive(consents, item.subjectId, 'marketing', at)) continue;
    sent.push(item.subjectId);
  }
  world.sent.push(...sent);
  return sent;
}
...
export function runFindings(): Finding[] {
  const p1 = { naive: afterErase(naiveErase), fixed: afterErase(fixedErase) };
  const p2 = { naive: afterPurge(naivePurge), fixed: afterPurge(fixedPurge) };
  const p3 = { naive: afterExport(naiveExport), fixed: afterExport(fixedExport) };
  const p4 = { naive: afterRevoke(naiveDispatch), fixed: afterRevoke(fixedDispatch) };

  return [
    {
      id: 'P1',
      label: 'delete-not-propagated',
      naive: `residual=${p1.naive.residual} at=[${p1.naive.locations.join(', ')}]`,
      fixed: `residual=${p1.fixed.residual} at=[${p1.fixed.locations.join(', ')}]`,
      reproduced: p1.naive.residual > 0,
      remains: p1.fixed.residual > 0,
    },
    {
      id: 'P2',
      label: 'retention-not-enforced',
      naive: `expired-left=${p2.naive.remaining} purged=${p2.naive.purged}`,
      fixed: `expired-left=${p2.fixed.remaining} purged=${p2.fixed.purged}`,
      reproduced: p2.naive.remaining > 0,
      remains: p2.fixed.remaining > 0,
    },
    {
      id: 'P3',
      label: 'export-includes-others',
      naive: `rows=${p3.naive.total} foreign=${p3.naive.foreign} missing=${p3.naive.missing}`,
      fixed: `rows=${p3.fixed.total} foreign=${p3.fixed.foreign} missing=${p3.fixed.missing}`,
      reproduced: p3.naive.foreign > 0 || p3.naive.missing > 0,
      remains: p3.fixed.foreign > 0 || p3.fixed.missing > 0,
    },
    {
      id: 'P4',
      label: 'consent-revoked-but-sent',
      naive: `sent=[${p4.naive.join(', ')}]`,
      fixed: `sent=[${p4.fixed.join(', ')}]`,
      reproduced: p4.naive.includes('S1'),
      remains: p4.fixed.includes('S1'),
    },
  ];
}
