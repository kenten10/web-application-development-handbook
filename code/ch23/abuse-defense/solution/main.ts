// Solution for 23.10 課題23.10: 濫用対策の鍵と応答の設計を破って塞ぐ (★★★)
...
export class LayeredGuard implements Guard {
  ...
  private signals(request: LoginRequest): Array<{ key: string; limit: number }> {
    ...
    return [
      { key: `login:acct:${email}`, limit: ACCOUNT_LIMIT },
      { key: `login:ip:${request.ip}`, limit: IP_LIMIT },
      { key: `login:net:${networkOf(request.ip)}`, limit: NETWORK_LIMIT },
      { key: 'login:global', limit: GLOBAL_LIMIT },
    ];
  }

  login(request: LoginRequest, trace: Trace): LoginOutcome {
    const signals = this.signals(request);
    const exceeded = signals.filter((signal) => this.counter.get(signal.key) >= signal.limit);
    if (exceeded.length >= 2) {
      // 判定がDBを引かずに完結するため、攻撃時でも安く返せる (23.27)
      return {
        status: 429,
    ...
    const challenged = exceeded.length === 1;

    const account = accountFor(request.email);
    // 存在しない場合も同じ経路を通す。処理時間の差を残さない (13.25)
    trace.verifiedHash = true;
    const ok = account !== undefined
      && account.password === request.password
      && !BREACHED.has(request.password);

    if (ok && !challenged) {
      for (const signal of signals) if (signal.key.startsWith('login:acct:')) this.counter.reset(signal.key);
      return { status: 200, body: 'authenticated', challenged: false, headers: {} };
    }
    for (const signal of signals) this.counter.increment(signal.key);
    if (challenged) {
      // 合図が立っているときは、成否にかかわらず同じ応答で追加確認へ回す。
      // 文言を成否で分けると、そこから正しい資格情報が判別できてしまう。
      // 締め出しではなく、本人が越えられる関門であることが要点である (13.25)。
      return { status: 401, body: 'additional verification required', challenged: true, headers: {} };
    }
    // 存在の有無で文言を変えない
    return { status: 401, body: 'invalid credentials', challenged: false, headers: {} };
  }
...
export function runFindings(): Finding[] {
  const b1 = { naive: credentialStuffing(new NaiveGuard()), layered: credentialStuffing(new LayeredGuard()) };
  const b2 = { naive: enumerate(new NaiveGuard()), layered: enumerate(new LayeredGuard()) };
  const b3 = { naive: lockoutDos(new NaiveGuard()), layered: lockoutDos(new LayeredGuard()) };
  const naiveBrute = bruteForceFromOneIp(new NaiveGuard());
  const layeredBrute = bruteForceFromOneIp(new LayeredGuard());
  const naiveHeaders = new NaiveGuard();
  const layeredHeaders = new LayeredGuard();
  bruteForceFromOneIp(naiveHeaders);
  bruteForceFromOneIp(layeredHeaders);
  const naiveRetry = naiveHeaders.login(
    { email: 'aoi@example.test', password: 'x', ip: '203.0.113.7' },
    { verifiedHash: false },
  );
  const layeredRetry = layeredHeaders.login(
    { email: 'aoi@example.test', password: 'x', ip: '203.0.113.7' },
    { verifiedHash: false },
  );
  ...
  return [
    {
      ...
      naive: `accepted=${b1.naive.accepted.length} [${b1.naive.accepted.join(', ')}]`,
      layered: `accepted=${b1.layered.accepted.length} [${b1.layered.accepted.join(', ')}]`,
      reproduced: b1.naive.accepted.length > 0,
      remains: b1.layered.accepted.length > 0,
    },
    {
      id: 'B2',
      label: 'user-enumeration',
      naive: `distinguishable=${b2.naive.distinguishable}/2`,
      layered: `distinguishable=${b2.layered.distinguishable}/2`,
      reproduced: b2.naive.distinguishable > 0,
      remains: b2.layered.distinguishable > 0,
    },
    {
      id: 'B3',
      label: 'lockout-dos',
      naive: `victim-blocked=${b3.naive.victimBlocked}`,
      layered: `victim-blocked=${b3.layered.victimBlocked}`,
      reproduced: b3.naive.victimBlocked,
      remains: b3.layered.victimBlocked,
    },
    {
      id: 'B4',
      label: 'missing-retry-after',
      naive: `blocked=${naiveBrute.blocked}/${naiveBrute.attempts} status=${naiveRetry.status} retry-after=${naiveRetry.headers['retry-after'] ?? 'none'}`,
      layered: `blocked=${layeredBrute.blocked}/${layeredBrute.attempts} status=${layeredRetry.status} retry-after=${layeredRetry.headers['retry-after'] ?? 'none'}`,
      reproduced: naiveRetry.headers['retry-after'] === undefined,
      remains: layeredRetry.headers['retry-after'] === undefined,
    },
  ];
}
