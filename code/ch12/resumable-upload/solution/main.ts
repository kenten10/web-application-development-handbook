// 模範解答 for 12.6 課題12.6: 再開可能アップロードの中断を再現して直す (★★★)
// 本文 12.13 の署名条件と、12.14 のオフセット条件・回収を、動くコードとして示す。
//
// 設計の要点:
//   - セッション台帳 (declaredBytes / expiresAt / key) は「DBの行」に相当し、再起動で消えない。
//   - 受信済みオフセットは「保存済みバイト列の実長」から毎回導く。台帳にも持たせない。
//     こうすると、追記とオフセット更新の不整合が原理的に起きない。
//   - 上限は許可証 (Grant) の maxBytes としてストレージへ渡す。ここに入れた値だけが強制される。
...
/**
 * 許可証を発行する。
 * fixed は上限を許可証へ含めるため、ストレージ側で強制される。
 * naive は台帳に控えるだけで、書き込みを縛らない。
 */
export function issueGrant(server: Server, request: { declaredBytes: number; ttlMs: number; now: number }): Grant {
  ...
  const maxBytes = server.mode === 'fixed' ? request.declaredBytes : null;
...
/**
 * 受信済みバイト数を返す (tus の HEAD に相当)。
 * fixed は保存済みバイト列の実長から導くため、再起動やノード切り替えでも同じ値になる。
 */
export function headSession(server: Server, sessionId: string): number {
  const row = sessionOf(server, sessionId);
  if (server.mode === 'fixed') return server.storage.length(row.key);
  return server.memoryOffset.get(sessionId) ?? 0;
}

/**
 * 追記する (tus の PATCH に相当)。
 * fixed はオフセットの一致を書き込みの条件にするため、同じ PATCH の再送が二重に書かれない。
 */
export function patchChunk(server: Server, sessionId: string, offset: number, chunk: Uint8Array): number {
  const row = sessionOf(server, sessionId);
  if (server.mode === 'fixed') {
    const received = server.storage.length(row.key);
    if (offset !== received) {
      throw new ConflictError(`offset mismatch: server=${received} client=${offset}`);
    }
    if (offset + chunk.byteLength > row.declaredBytes) {
      throw new PayloadTooLargeError('exceeds declared length');
    }
  }
...
/** 期限切れの未完了セッションを回収する。naive は何もしない。 */
export function collectExpired(server: Server, now: number): Collected {
  if (server.mode === 'naive') return { sessions: 0, bytes: 0 };
  let sessions = 0;
  let bytes = 0;
  for (const row of [...server.sessions.values()]) {
    if (row.expiresAt > now) continue;
    if (server.storage.length(row.key) >= row.declaredBytes) continue; // 完了済みは残す
    bytes += server.storage.remove(row.key);
    server.sessions.delete(row.id);
    server.memoryOffset.delete(row.id);
    sessions += 1;
  }
...
export const FIXTURES = {
  ...
  /**
   * U3: 8MiB を送る途中で応答を取りこぼし、同じチャンクを再送する。
   * 宣言長は 16MiB にしてある。長さの上限で偶然止まるのではなく、
   * オフセット条件そのものが二重書き込みを止めていることを見るためである。
   */
  duplicate: { declaredBytes: 16 * MIB, totalBytes: 8 * MIB, dropAckAtBytes: 8 * MIB },
...
export function probeDuplicate(mode: Mode): { stored: number; sent: number } {
  const server = createServer(mode);
  const { declaredBytes, totalBytes, dropAckAtBytes } = FIXTURES.duplicate;
  const grant = issueGrant(server, { declaredBytes, ttlMs: FIXTURES.ttlMs, now: FIXTURES.now });
  ...
  return { stored: server.storage.length(grant.key), sent: totalBytes };
}
...
export function runFindings(): Finding[] {
  const oversize = { naive: probeOversize('naive'), fixed: probeOversize('fixed') };
  const resumed = { naive: probeResume('naive'), fixed: probeResume('fixed') };
  const duplicate = { naive: probeDuplicate('naive'), fixed: probeDuplicate('fixed') };
  const orphan = { naive: probeOrphan('naive'), fixed: probeOrphan('fixed') };

  return [
    {
      id: 'U1',
      label: 'signed-size-ignored',
      naive: `stored=${oversize.naive.stored}`,
      fixed: `stored=${oversize.fixed.stored} (declared=${oversize.fixed.declared})`,
      reproduced: oversize.naive.stored > oversize.naive.declared,
      remains: oversize.fixed.stored > oversize.fixed.declared,
    },
    {
      id: 'U2',
      label: 'resume-restart',
      naive: `resent=${resumed.naive.resent}`,
      fixed: `resent=${resumed.fixed.resent} (minimum=${resumed.fixed.minimum})`,
      reproduced: resumed.naive.resent > resumed.naive.minimum,
      remains: resumed.fixed.resent > resumed.fixed.minimum,
    },
    {
      id: 'U3',
      label: 'duplicate-chunk',
      naive: `stored=${duplicate.naive.stored}`,
      fixed: `stored=${duplicate.fixed.stored} (sent=${duplicate.fixed.sent})`,
      reproduced: duplicate.naive.stored !== duplicate.naive.sent,
      remains: duplicate.fixed.stored !== duplicate.fixed.sent,
    },
    {
      id: 'U4',
      label: 'orphan-session',
      naive: `retained=${orphan.naive.retained.sessions}/${orphan.naive.retained.bytes}B`,
      fixed: `retained=${orphan.fixed.retained.sessions}/${orphan.fixed.retained.bytes}B (collected=${orphan.fixed.collected.sessions}/${orphan.fixed.collected.bytes}B)`,
      reproduced: orphan.naive.retained.sessions > 0,
      remains: orphan.fixed.retained.sessions > 0,
    },
  ];
}
