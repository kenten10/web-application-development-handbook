// Starter for 6.6 課題6.6: フォーカスとエラー通知の欠落を再現して塞ぐ (★★★)
// 本文 6.11 (フォーカス管理) と 7.9 (フォームのアクセシビリティ) の判断を、
// ブラウザを使わずに検証できる最小の文書モデルの上で再現する。
//
// 手順:
//   1. buildScreen の文書と naiveDialog / naiveForm を読み、4つの障壁が何かを言葉にする。
//   2. tabbables を実装する。以降の判定はすべてこの結果の上に載る。
//   3. fixedDialog の open / close / tab を実装する (6.11 の4つの動作)。
//   4. fixedForm.submit を実装する (7.9 の3経路)。
//   5. runFindings を実装し、report.ts を通す。
//
// 完成したら次を実行する。
//   pnpm --filter @handbook/ch06 exec tsx a11y-focus/starter/report.ts
//
...
export function tabbables(root: El): El[] {
  // TODO: 上の4つの規則を実装する。walk(root) の順序が文書順である。
  //       blockedBy(root, node, 'hidden') と blockedBy(root, node, 'inert') が使える。
  void root;
  return [];
}
...
export const fixedDialog: Dialog = {
  open(screen) {
    // TODO: 6.11 の4つの動作のうち 1・2・3 を実装する。
    //   1. returnTo へ開く前の activeId を記憶する
    //   2. ダイアログを表示し、その中の最初の tabbable へ移す (無ければ見出しへ)
    //   3. 背後を inert にする。aria-hidden だけでは Tab が抜ける
    void screen;
  },
  close(screen) {
    // TODO: 4 を実装する。ダイアログを隠し、inert を外し、記憶した位置へ戻す。
    //       戻り先が消えている場合は page-title に tabindex="-1" を付けてそこへ移す。
    void screen;
  },
  tab(screen, shift = false) {
    // 背後を inert にできていれば、moveNext だけでダイアログ内を巡回する
    moveNext(screen, shift);
  },
};
...
export const fixedForm: Form = {
  submit(screen, values) {
    // TODO: 7.9 の3経路を実装する。
    //   経路1 フィールド単位: email-error を追加し、入力欄へ aria-invalid と
    //         aria-describedby (email-help と email-error の両方) を付ける
    //   経路2 まとめ: 件数を含む見出しと、各欄へのリンクを持つ error-summary を先頭へ置く
    //   経路3 通知: error-summary (tabindex="-1") へフォーカスを移す
    //   エラーが無い場合は announce() で成功を通知する
    clearErrorNodes(screen);
    const errors = validate(values);
    return { errors, ok: Object.keys(errors).length === 0 };
  },
 ...
 * reproduced は「naive で障壁が再現したか」、remains は「fixed にも残るか」。
 * focusAfterOpen / escapedIds / focusAfterDelete / errorDelivery をそのまま使える。
 */
